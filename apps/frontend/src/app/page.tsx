...
export default function HomePage() {
  ...
  const handleFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      setCsv(text) // ubacujemo CSV u state koji backend već očekuje
    }
    ...
    <div className="min-h-screen p-6 bg-gray-400 text-white space-y-6">
      ...
      <div className="space-y-3 flex flex-col">
        ...
        <input
          ...
          accept=".csv"
          onChange={handleFile}
          className="w-60 h-16 p-3 rounded-lg bg-gray-800 border border-gray-700"
        />
        {/* <button
        ...
        <button
          onClick={handleUpload}
          className="w-60 h-20 flex items-center justify-center py-3 rounded-lg bg-linear-to-r from-(--primary) to-(--secondary) text-white font-semibold shadow-md hover:scale-101 active:scale-95 transform transition cursor-pointer"
        >
          Upload CSV
        </button>
      </div>
      ...
      {/* Member List */}
      <div className="space-y-4 flex w-full justify-around gap-4">
        {members?.map((m: any) => (
          <Link
            href={`/members/${m.id}`}
            key={m.id}
            className="flex-1 h-36 p-4 rounded-lg bg-gray-800 hover:bg-gray-700 shadow-md transition"
          >
